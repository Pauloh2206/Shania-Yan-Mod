...
// O EmojiMix é opcional. Não interrompa o carregamento dos demais comandos
// quando a chave Tenor não estiver configurada.
const client = CONFIG.API.KEY ? new TenorClient(CONFIG.API.KEY) : null;
...
async function emojiMix(emoji1, emoji2) {
    try {
        if (!client) {
            throw new EmojiMixError('Chave da API Tenor não configurada. Defina TENOR_API_KEY no arquivo .env para usar o EmojiMix.');
        }
